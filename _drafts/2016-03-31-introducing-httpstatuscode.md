---
layout: post
title: Introducint httpstatusco.de
tags: projects
excerpt: I've created httpstatusco.de, a small website that allows you to look up HTTP status codes. 
comments: true
---

Late last year I started working at a new client. The client was in the middle of building new and shiny microservices using REST and I joined them to help with this effort. I had dabbed around with REST and HTTP before and certainly knew my fair share of HTTP and the status codes that come with it. After all the beloved `404` has become a widespread meme a long time ago **TODO: cite**. 

Working extensively on microservices talking to each other via REST, however, quickly showed that there were some rather obscure HTTP status codes that I hadn't heard about before. Sometimes we encountered HTTP status codes in our daily work that were rather unknown to us. Wikipedia's List of HTTP Status Codes **TODO: link** was one of my most visited bookmarks during that time. Sometimes I really enjoyed learning about the trivia of certain obscure HTTP status codes. I decided to make a game out of it to share what I had learned with my fellow devs -- after all I was not the only one who couldn't recite [RFC 7231](https://tools.ietf.org/html/rfc7231) from the top of my head. I started declaring the _"status code of the day"_ in our daily standup. I simply put up a sticky with a numeric status code and asked my team to guess what it meant in terms of HTTP. The game caught on and on consequent days my teammates started putting up status codes on their own that they stumbled accross the day before. It was a fun little excercise that had a certain learning effect.

**TODO: include image of sticky**

Fast forward a couple of weeks: I had lunch with some of my colleagues working in other teams and told them about this little trivia game we initiated. We got into a discussion about HTTP and how all of us had struggled with memorizing obscure status codes on time or another. This conversation sparked a thought in my head about making the Wikipedia list a little more accessible. It should really be at your fingertips to simply look up a simple status cod This conversation sparked a thought in my head about making the Wikipedia list a little more accessible. Looking up a simple status code should really be at your fingertips. And so an idea about a web service allowing this simple lookup started taking shape in my head.

As with every good pet project it started with buying a good domain first (something that Birgitta **TODO: link twitter** jokingly called _"Domain-Driven Development"_)  
